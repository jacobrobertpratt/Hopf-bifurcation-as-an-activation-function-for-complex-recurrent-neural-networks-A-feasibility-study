...
#            if 'mr' == tstact: self.activation = ModReLU()
    ...
    def build( self , input_shape ):
        ...
        self.mxlen = max( inshp[-1] , self.units )
#        print( self.name + ' mxlen' , self.mxlen )
        
        do_save = False
        do_train = True
        
        mtd = str( self.method ) + '_' + str( self.units )
    ...
    ## Method 1: Maps the state via matrix-matrix and input as matrix-vector ##
    @tf.function
...
#        _print( 'z_i' , z_i )
        ...
        if self.hopfact is not None:
            ...
            y_t = self.hopfact( z , z_i , ones , self.units , 1 )
...
#        _print( self.name + ' D) _input' , _input )
        '''
...
#        _print( self.name + ' _cstate' , _cstate )
        ...
        '''
        _output = _cout.transpose(( 1 , 2 , 0 )) * self.fftnorm
#        _print( self.name + ' _output' , _output )
...
#            _print( self.name + ' C) fft1 _output' , _output )
        
        _output = _output[::,0:self.osz,0:self.isz].transpose(( 0 , 2 , 1 ))
#        _print( self.name + ' D) _output' , _output )
        '''
...
#        _print( self.name + ' E) _output' , _output )
