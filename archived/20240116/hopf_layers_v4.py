...
#        _print( self.name + ' zn' , zn )
        ...
        if self.hopfact1 is not None:
            ...
            y_j = self.hopfact1( z_ , z_i , ones , self.units )
        elif self.hopfact2 is not None:
            ## ... 
            pass
        elif self.activation is not None:
            y_j = self.activation( z_i )
        else:
            y_j = z_i
...
#        _print( self.name + ' z_i' , z_i )
        ...
        re_j = ( z0 + x0 ) / 2.
        y_t = self.combine_output( re_j , y_j )
        z_k = self.combine_output( re_j , z_i )
        z_t = [ z_k ] if tf.nest.is_nested( states ) else z_k
#        _print( 'y_t' , y_t )
#        _print( 'z_t' , z_t )
...
class HopfRNNLayerTheta( tf.keras.layers.Layer ):
    ...
    def build( self , input_shape ):
        ...
        # Size of units that we put into the hopf cell -> add 1 for real-value vector
        ...
        # Calc max potential output of rfft function given input &|| state dimensions
        ...
        self.fftnorm = tf.constant( tf.cast( self.seqlen , dtype = self.dtype ) , dtype = self.dtype )
...
#        _print( self.name + ' A) state2' , self.state2 )
        
        _input = tf.signal.rfft( inputs , fft_length = [ self.seqlen ] )
#        _print( self.name + ' B) _input' , _input )
        
        _input = inputs[::,::,0:self.usz] / self.fftnorm
#        _print( self.name + ' C) _input' , _input )
