...
class HopfRNNCellTheta( tf.keras.layers.Layer ):
    ...
    def __init__(
        ...
        self.activation = HopfActivTheta() if activation is None else activation
...
#        exit(0)
        ...
        '''
        xwgtshp = tf.signal.rfft( tf.ones( shape = input_shape , dtype = self.dtype ) ).shape
        zwgtshp = tf.signal.rfft( tf.ones( shape = ( input_shape[0] , self.units ) , dtype = self.dtype ) ).shape
...
#        _print( 'N' , N )
        ...
        Mx = tf.linalg.matmul( inputs , M )
#        _print( 'Mx' , Mx )
        
#        Nz = tf.linalg.matmul( state , N )
        Nz = backend.dot( state , N )
        _print( 'Nz' , Nz )
