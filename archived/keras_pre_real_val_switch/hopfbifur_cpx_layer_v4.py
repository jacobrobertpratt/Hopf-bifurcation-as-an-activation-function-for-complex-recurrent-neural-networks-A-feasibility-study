...
class HopfBifurCpxRNNCell(tf.keras.layers.Layer):
    ...
    def build( self, input_shape ):
        ...
        self.B = self.add_weight(
            ...
            shape = ( ssz , ssz ),
            dtype = self.dtype,
            initializer = Hermitian( name = 'B' , save = self.do_save ), # , conjsym = self.do_conjsym ),
            trainable = do_train
        )
        ...
        self.G = self.add_weight(
            ...
            shape = ( ssz , ssz ),
            dtype = self.dtype,
            initializer = Hermitian( name = 'G' , save = self.do_save , conjsym = self.do_conjsym ),
            trainable = do_train
        )
        ...
        self.H = self.add_weight(
            ...
            shape = ( ssz , ssz ),
            dtype = self.dtype,
            initializer = Hermitian( name = 'H' , save = self.do_save , conjsym = self.do_conjsym ),
            trainable = do_train
        )
        
        self.ones = tf.constant( tf.ones( shape = ( ssz + 1 , 1 ), dtype = self.dtype ), dtype = self.dtype )
...
#        tf.print( '\n'+'- '*20+' Cell '+'- '*20+'\nCount:' , self.cnt[0] , '\n' )
        ...
        uz = self.mke_unt_vec( zn )
        ux = self.mke_unt_vec( xn )
        nz = self.mke_nrm_vec( zn )
        nx = self.mke_nrm_vec( xn )
        
        Az1 = self.hermitian_map( A , uz )
        Az2 = self.hermitian_map( tf.linalg.matrix_transpose( A ) , uz )
        
        Bx1 = self.hermitian_map( B , ux )
        Bx2 = self.hermitian_map( tf.linalg.matrix_transpose( B ) , ux )
        
        z0_j = tf.linalg.matmul( Az1 , Az2 , adjoint_a = True )
        x0_j = tf.linalg.matmul( Bx1 , Bx2 , adjoint_a = True )
...
#        tst1 = x_t * tf.math.conj( (1.+0.j) - self.mke_unt_vec( x_t ) )
#        _print( 'tst1' , tst1 , summarize = -1 )
...
#        state_init = np.random.rand( self.state_size )
#        state_init = np.arange( self.state_size )
#        state_init = ( state_init - state_init[::-1] ) / 2.
#        state_init = np.exp( 1.j*state_init ).reshape( -1 , 1 )
#        state_init = ( 2*np.pi * np.arange( self.state_size ) ) / self.state_size
#        state_init = np.cos( state_init ) + 1.j*np.sin( state_init )
        
        state_init = state_init.astype( np.complex64 )
        state_init = np.fft.fft( state_init ).reshape( -1 , 1 ) / self.state_size
        state_init = tf.reshape( tf.cast( state_init , dtype = self.dtype ) , [ ssz , 1 ] )
...
#        _print( 'input' , input )
        ...
        _input = tf.signal.fft( _input ) / self.input_size
...
#        tf.debugging.check_numerics( tf.math.real( _coutput ) , 'a _coutput element is nan' )
        ...
        _output = tf.signal.ifft( tf.linalg.matrix_transpose( _cout ) ) * self.input_size
#        _output = tf.linalg.matmul( _cout , _cstate , adjoint_a = True )
#        _output = _cout
