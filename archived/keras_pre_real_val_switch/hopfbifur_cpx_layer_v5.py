...
class HopfBifurCpxRNNCell(tf.keras.layers.Layer):
    ...
    def build( self, input_shape ):
        ...
        self.ones = tf.constant( tf.ones( shape = ( ssz , 1 ), dtype = self.dtype ), dtype = self.dtype )
    ...
    def test_unit_mat( self , U ):
        ichkmax = tf.math.reduce_sum( tf.math.abs( self.eye - tf.linalg.matmul( U , U , adjoint_b = True ) ) )
...
#        _print( 'x' , x )
        ...
        uz = self.mke_unt_vec( z )
        ux = self.mke_unt_vec( x )
        nz = self.mke_nrm_vec( z )
        nx = self.mke_nrm_vec( x )
...
#        if self.output_size is None: self.output_size = self.input_size
        ...
        if do_save:
            ...
            if not os.path.exists('state_layer.npy'):
                rand_init = np.random.rand( ssz )
                np.save( 'state_layer.npy' , rand_init , allow_pickle = True )
            else:
                rand_init = np.load( 'state_layer.npy' , allow_pickle = True )
        else:
            rand_init = np.random.rand( ssz )
            
        rand_init = ( ( rand_init + rand_init[::-1] ) / 2. )
        exp_init = np.linspace( -ssz//2 , ssz//2 , ssz ) / np.sqrt( 2*ssz + 1 )
        exp_init = np.exp( 2.j*np.pi * exp_init )
        state_init = rand_init.reshape( -1 , 1 ) * exp_init.reshape( -1 , 1 )
...
#        _print( 'mag( state )' , tf.math.abs( self.state ) )
#        _print( 'ang( state )' , tf.math.angle( self.state ) )
#        exit(0)
        
        inrng = tf.range( isz )
#        inshp = isz // 2
#        inrng = tf.linspace( -inshp , inshp , isz )
        inrng = tf.cast( tf.reshape( [ inrng ] , [ 1 , isz ] ) , dtype = self.dtype )
        self.inrng = tf.constant( inrng , dtype = self.dtype , name = 'inrng_' + self.name )
#        _print( 'inrng' , self.inrng )
        
        outrng = tf.range( osz )
#        outshp = osz // 2
#        outrng = tf.linspace( -outshp , outshp , osz )
        outrng = tf.cast( tf.reshape( [ outrng ] , [ 1 , osz ] ), dtype = self.dtype )
        self.outrng = tf.constant( outrng , dtype = self.dtype , name = 'outrng_' + self.name )
#        _print( 'outrng' , self.outrng )
#        exit(0)
...
#        _print( 'input' , input )
        ...
        z = self.state
        x = tf.cast( input , dtype = self.dtype )
        
        uz = self.mke_unt_vec( z )
        b_i = tf.math.pow( uz , self.inrng )
        x_i = tf.linalg.matmul( b_i , x , adjoint_b = True ) / self.input_size
#        _print( 'b_i' , b_i )
#        _print( 'x_i' , x_i )
...
#        _output = tf.linalg.matmul( _cout , _cstate , adjoint_a = True )
        ...
        uz_t = self.mke_unt_vec( z_t )
        b_o = tf.math.pow( uz_t , self.outrng ) * ( tf.math.abs( z_t ) + 0.j )
#        _print( 'b_o' , b_o )
        
        y_t = tf.linalg.matmul( x_t , b_o , adjoint_a = True )
        _print( 'y_t' , y_t )
        
        max_img_val = tf.math.reduce_max( tf.math.imag( y_t ) )
        tf.print('\nmaximg:', max_img_val )
...
#        self.state.assign( _cstate )
        y_t = tf.cast( tf.math.real( y_t ) , input.dtype )
#        y_t = tf.reshape( y_t , input.shape )
#        _print( 'y_t' , y_t )
