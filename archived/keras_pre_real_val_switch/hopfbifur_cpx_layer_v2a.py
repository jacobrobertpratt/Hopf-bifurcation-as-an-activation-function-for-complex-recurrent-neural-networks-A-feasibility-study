...
class HopfBifurCpxRNNCell(tf.keras.layers.Layer):
    ...
    def build( self, input_shape ):
        ...
        self.G = self.add_weight(
            ...
            shape = ( ssz , ssz ),
            dtype = self.dtype,
            initializer = Hermitian( name = 'G' , save = do_save , conjsym = self.do_conjsym ),
            trainable = do_train
        )
        ...
        self.H = self.add_weight(
            ...
            shape = ( ssz , ssz ),
            dtype = self.dtype,
            initializer = Hermitian( name = 'H' , save = do_save , conjsym = self.do_conjsym ),
            trainable = do_train
        )
...
#        tf.print( '\n'+'- '*20+' Cell '+'- '*20+'\nCount:' , self.cnt[0] , '\n' )
        ...
        uz = tf.math.divide_no_nan( z , tf.cast( tf.math.abs( z ) , dtype = z.dtype ) )
        ux = tf.math.divide_no_nan( x , tf.cast( tf.math.abs( x ) , dtype = x.dtype ) )
        nz , _ = tf.linalg.normalize( z , ord = 2 )
        nx , _ = tf.linalg.normalize( x , ord = 2 )
        
        Gz = self.hermitian_map( G , nz )
        iGz = self.unitary_map( 1.j*G , nz )
        
        Hx = self.hermitian_map( H , nx )
        iHx = self.unitary_map( 1.j*H , nx )
