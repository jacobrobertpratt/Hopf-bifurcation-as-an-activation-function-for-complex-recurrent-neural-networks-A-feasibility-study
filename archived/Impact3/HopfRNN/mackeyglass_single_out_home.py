...
#    print('y_glass:\n',y_glass,'\nshape:',y_glass.shape,'  dtype:',y_glass.dtype,'\n')
    ...
    train = np.reshape(train,(tbsz,1,feature_size))
...
#    exit(0)
    ...
    vbsz = int(valid_size-feature_size)
    vbatch = []
    for s in range(vbsz):
        e = s + feature_size
        vbatch.append(list(valid[s:e]))
    valid = np.asarray(vbatch)
    valid = np.reshape(valid,(vbsz,1,feature_size))
