...
class MyRNNCell(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        # Unitary dft
        init_udft = init_dft / np.sqrt(in_sz)
        self.udft = tf.constant(init_udft,dtype=tf.complex128)
...
#        U = unitary_group.rvs(mini_sz)
#        U = scipy.linalg.logm(U)
        U = (U + tf.math.conj(U)[::-1,::-1])/2.                 # Generates a skew-hermitian with zero trace
#        U = U / np.power(np.linalg.det(U),(1/mini_sz))         # Generates a determinant equal to 1 matrix
        self.U = tf.Variable( U,
                              name='U_'+str(self.cell_id),
                              dtype=tf.complex128,
                              trainable=False
                             )
#        _print_matrix('self.U',self.U)
#        exit(0)
        
#        _print_matrix('U',self.U)
#        MU = tf.linalg.expm(self.U)
#        _print_matrix('MU',MU)
#        exit(0)
        # Properties of above U matrix
        #   1) Has equal and opposite imaginary eigen-values
        #   2) Has a determinant = 1 and trace = 0
        
        
        if os.path.exists('rnn_mat_v.npy'):
            V = np.load('rnn_mat_v.npy',allow_pickle=True)
            # Check saved shape and replace if not the same.
            if V.shape[-1] != mini_sz:
                V = unitary_group.rvs(mini_sz)
                V = scipy.linalg.logm(V)
                np.save('rnn_mat_v.npy',V,allow_pickle=True)
        else:
            V = unitary_group.rvs(mini_sz)
            V = scipy.linalg.logm(V)
            np.save('rnn_mat_v.npy',V,allow_pickle=True)
...
#        V = unitary_group.rvs(mini_sz)
#        V = scipy.linalg.logm(V)
        V = (V + tf.math.conj(V)[::-1,::-1])/2.                 # Generates a skew-hermitian with zero trace
#        V = V / np.power(np.linalg.det(V),(1/mini_sz))      # Generates a determinant equal to 1 matrix
        ...
        if os.path.exists('rnn_vec_z.npy'):
            ...
            # Check saved shape and replace if not the same.
            if z.shape[0] != self.in_sz:
                z = np.random.rand(in_sz).reshape(-1,1)
#                z = np.exp(1.j*z) * np.random.rand(in_sz).reshape(-1,1)
...
#            z = np.exp(1.j*z) * np.random.rand(in_sz).reshape(-1,1)
        ...
        MH = unitary_group.rvs(in_sz)
        H = tf.linalg.diag((np.arange(in_sz)+1)/in_sz)
        H = tf.math.conj(MH).T @ H @ MH
        self.H = tf.Variable( H,
                              name='H_'+str(self.cell_id),
                              dtype=tf.complex128,
                              trainable=True
                             )
...
#        z = np.random.rand(in_sz).reshape(-1,1)
#        z = np.exp(1.j*z) * np.random.rand(in_sz).reshape(-1,1)
#        z = (z + tf.math.conj(tf.expand_dims(z.T[0][::-1],1)))/2.       # Make a conj-symmetric vector #
...
#        exit(0)
        ...
        onezrs = np.zeros(shape=(in_sz))
        onezrs[0] = 1
        onezrs = onezrs.reshape(-1,1)
        self.onezrs = tf.constant(onezrs,dtype=tf.complex128)
#        _print('onezrs',onezrs)
#        exit(0)
        
        zrones = np.ones(shape=(in_sz))
        zrones[0] = 0
        zrones = zrones.reshape(-1,1)
        self.zrones = tf.constant(zrones,dtype=tf.complex128)
#        _print('zrones',zrones)
#        exit(0)
...
#        _print('dz',dz)
    ...
    ''' MyRNN '''
    @tf.function
    def calc_cpx_Hopf_NxN(self,z,A,B):
...
#        tf.print('\n-----------------------------------------------------------------------------\n')
        ...
        Izz = self.eye * (tf.math.conj(z).T @ z)
#        _print('Izz',Izz)
#        _cpx_eigvals('Izz',Izz)
#        _cpx_pdet('Izz',Izz)
#        _cpx_ptrace('Izz',Izz)
...
#        _print('dz',dz)
    ...
    ''' MyRNN '''
    @tf.function
    def hopf_Diff_EQ_NxN(self,t,state,re_A,im_A,re_B,im_B):
        re_z, im_z = tf.unstack(state)
        z_stk = tf.stack([re_z,im_z])
        A_stk = tf.stack([re_A,im_A])
        B_stk = tf.stack([re_B,im_B])
        re_dz, im_dz = tf.unstack(self.calc_cpx_Hopf_NxN(z_stk,A_stk,B_stk))
        return tf.stack([re_dz,im_dz])
    
    ''' MyRNN '''
    @tf.function
    def hopf_ODE_NxN(self,z,A,B):
...
#        _print('dft_x',dft_x)
        ...
        prjN = (z @ tf.math.conj(dft_x).T)/(tf.math.conj(z).T @ z)
        prjO = self.eye - prjN
        
        z_0 = z[1::]
#        _print('z_0',z_0)
        
        inr = tf.math.conj(z).T @ (prjN @ H) @ dft_x
        mag = self.onezrs * inr
#        _print('inr',inr)
#        _print('mag',mag)
        ...
        _a = tf.math.abs(map)
        a = tf.complex(tf.math.real(map),tf.zeros_like(_a))
#        _print('a',a)
        
#        b = self.ones[1::]
        b = tf.complex(tf.ones_like(_a),tf.math.imag(map))
#        _print('b',b)
        
        # Map to the limit cycle #
        z_t = self.hopf_ODE(z_0,a,b)
#        z_t = self.hopf_ODE_NxN(z_0,A,B)
#        _print('z_t',z_t)
        
        cat = tf.concat([inr,z_t],0)
#        _print('cat',cat)
        
        y_t = tf.math.conj(cat).T @ self.udft
        _print('y_t',y_t)
        
        # Pulling the real-value and re-mapping it to the DFT fixes floating point erros that explode the model.
        self.z.assign(self.udft @ tf.math.real(y_t.T))
...
#        self.cells = [MyRNNCell(self.state_size,cell_id=j,name='rnncell_'+str(j)) for j in range(self.state_size)]
    ...
    ''' MyRNN '''
    @tf.custom_gradient
    def state_out_phase_map(self,z):
        
#        z = (z + tf.math.conj(z)[::-1])/2.    # Clean up the results
        
        M = tf.math.exp(1.j*tf.math.angle(z))**np.arange(self.in_sz)
        cpx_y_hat = tf.math.conj(z).T @ M
#        _print('cpx_y_hat',cpx_y_hat)
        y_hat = tf.math.real(cpx_y_hat)             # All imaginary values in cy_hat are zero.
        def grad(dL):
            dz = M @ tf.transpose(dL)
            return dz
        return y_hat, grad
...
#        y_hat = self.state_out_phase_map(z_t)
#        _print('y_hat',y_hat)
        
        ''' UPDATE AUXILIARY VARIABLES '''
        self.counter.assign_add([self.in_sz])
        
        return tf.reshape(y_t,_input.shape)
