...
def _cpx_ptrace(msg='',mat=None):
    ...
    tf.print('\n'+str(msg),tf.math.real(trc),tf.math.imag(trc))
    tf.print()

def _cpx_pdet(msg='',mat=None):
    if mat is None:
        return
    tf.print(msg + ' Determinant:')
    det = tf.linalg.det(mat)
    tf.print('\n'+str(msg),tf.math.real(det),tf.math.imag(det))
    tf.print()
...
class MyRNNCell(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        if os.path.exists('rnn_mat_u.npy'):
            ...
            if U.shape[-1] != self.in_sz:
                U = unitary_group.rvs(in_sz)
...
#        U = tf.linalg.expm(U)
        ...
        if os.path.exists('rnn_mat_v.npy'):
            ...
            # Check saved shape and replace if not the same.
            if V.shape[-1] != self.in_sz:
                V = unitary_group.rvs(in_sz)
...
#    @tf.custom_gradient
    @tf.function
    def input_map(self,M,v):
        EM = tf.linalg.expm(M)
        EMv = EM @ v
        Wv = (EMv + tf.math.conj(EMv)[::-1])/2.
        '''
        def grad(df):
            A = (v @ tf.math.conj(df).T - df @ tf.math.conj(v).T)/2.
#            dW = tf.linalg.inv(self.eye + A/2.) @ (self.eye - A/2.)
#            dW = dW @ W
#            dW = tf.linalg.logm(dW)
#            _print('M',M)
#            _print('dW',dW)
            dM = A # dW # tf.zeros_like(M)
            dv = tf.zeros_like(v)
            return dM, dv
        '''
        return Wv #, grad
...
#    @tf.custom_gradient
    @tf.function
    def state_map(self,M,v):
        W = tf.linalg.expm(M)
        Wv = W @ v
        Wv = (Wv + tf.math.conj(Wv)[::-1])/2.
        '''
        def grad(df):
            A = (v @ tf.math.conj(df).T - df @ tf.math.conj(v).T)/2.
#            dW = tf.linalg.inv(self.eye + A/2.) @ (self.eye - A/2.)
...
#        _print('z',z)
        ...
        # Map the input 'x' to the learned input mapping
        Vx = self.input_map(V,x)
#        _print('Vx',Vx)
        
        # Map the state 'z' to the new state position
        Uz = self.state_map(U,z)
#        Uz = U @ z
#        Uz = (Uz + tf.math.conj(Uz)[::-1])/2.
#        _print('Uz',Uz)
        
        div = (Vx*tf.math.conj(Uz)) - (tf.math.conj(Uz)*Uz)
#        _print('div',div)
        
#        ang_Uz = tf.math.angle(Uz)
#        ang_Vx = tf.math.angle(Vx)
#        _print('ang_Uz',ang_Uz)
#        _print('ang_Vx',ang_Vx)
        
#        mag_z = tf.math.abs(z)
#        mag_Vx = tf.math.abs(Vx)
#        _print('mag_Uz',mag_Uz)
#        _print('mag_Vx',mag_Vx)
        
#        mag_dif = (mag_Vx - mag_z)/2.
#        cpx_mag = tf.complex(mag_dif,tf.zeros_like(mag_dif))
        
#        ang_dif = ang_Vx - ang_Uz
#        cpx_ang = tf.complex(tf.zeros_like(ang_dif),ang_dif)
        
        a = tf.math.abs(div) # self.ones
        b = self.ones
#        _print('a',a)
#        _print('b',b)
...
#        z_t = self.round_err(z_t)       # Used for removing rounding errors (no actual gradient, but if gradient isn't conj-symm then it makes it conj-symm).
#        _print('z_t',z_t)
...
#        self.cells = [MyRNNCell(self.state_size,cell_id=j,name='rnncell_'+str(j)) for j in range(self.state_size)]
    ...
    def state_out_phase_map(self,z):
        M = tf.math.exp(1.j*tf.math.angle(z))      # **np.arange(2,self.in_sz+2)
...
#        _print('y_hat',y_hat)
        ...
        return y_hat # tf.reshape(y_hat,_input.shape)
...
#            _print('MyModel -> loss',loss)
            ...
            if self.my_loss is not None:
                self.results['loss'] = tf.math.reduce_sum(loss)
