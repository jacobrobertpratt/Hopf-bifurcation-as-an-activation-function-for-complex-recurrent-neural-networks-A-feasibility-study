...
def _cpx_ptrace(mat,msg='Trace:'):
    trc = tf.linalg.trace(mat)
    tf.print('\n'+str(msg),tf.math.real(trc),tf.math.imag(trc))
    tf.print()

def _cpx_eigvals(mat,msg='Eigen Values:'):
    tf.print('\n'+str(msg)+'\n')
...
class MyRNNCell(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        H = tf.cast(unitary_group.rvs(in_sz),dtype=tf.complex128)
        H_diag = tf.cast(np.diag(np.random.rand(in_sz)),dtype=tf.complex128)
        H = H @ (H_diag @ tf.math.conj(H).T)
        H = (H + H.T[::-1,::-1])/2.
...
#        tf.print('\n-----------------------------------------------------------------------------\n')
        ...
        Izz = B @ (self.eye * (tf.math.conj(z).T @ C @ z)) @ tf.math.conj(B).T
#        _print('Izz',Izz)
        
        W = A - Izz
        _print('W',W)
        _cpx_eigvals(W,'Eigen Values(W):')
        _cpx_ptrace(W,'Trace(W):')
...
#        rng = np.arange(2.*np.pi/self.in_sz,2.*np.pi,2.*np.pi/(self.in_sz+1))
        rng = np.arange(1,self.in_sz+1)
        M = (tf.math.exp(-1.j*tf.math.angle(self.z))**rng)
        Mx = M @ x_in
#        _print('Mx',Mx)
        
        E = (-1.j)*tf.linalg.expm(1.j*H)
