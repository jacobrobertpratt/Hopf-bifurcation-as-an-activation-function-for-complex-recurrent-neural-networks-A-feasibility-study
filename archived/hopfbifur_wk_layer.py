...
from activations import HopfBifurBase

@tf.keras.utils.register_keras_serializable('hopf_cell')
class HopfRNNCell( tf.keras.layers.Layer ):
    '''
    ...
    def __init__(
        ...
        super( HopfRNNCell , self ).__init__( **kwargs )
        
        self.state_size = state_size
        
        self.activation = HopfBifur() if activation is None else activation
    ...
    def build( self , input_shape ):
        ...
        self.A = self.add_weight(
            shape = ( self.state_size , input_shape[0] ),
        ...
        super( HopfRNNCell , self ).build( input_shape )
...
#        _print( 'input', input )
        ...
        z_stk = states[0] if tf.nest.is_nested( states ) else states
#        _print( 'states' , z_stk )
        ...
        ## Split State ##
        z_lst = tf.unstack( tf.expand_dims( z_stk , -1 ) )
        z_0 , z_n = z_lst[0] , z_lst[1::]
        x_0 = input
        
        x_i = tf.linalg.matmul( A , x_0 )
#        _print( 'x_i' , x_i )
        
        z_i = tf.linalg.matmul( B , z_0 )
#        _print( 'z_i' , z_i )
        
        z_j = x_i + z_i
#        _print( 'z_j' , z_j )
        
#        y_t = tf.keras.activations.tanh( z_j )
#        y_t = tf.keras.activations.relu( z_j )
#        y_t = self.activation( z_j , x_i * z_i )
#        y_t = self.activation( z_j , tf.math.pow( z_j , 2 ) )
#        _print( 'tst' , tst )
        
        ## Combine State ##
        z_t = tf.squeeze( tf.stack( z_n + [ z_j ] ) , -1 )
        z_t = [ z_t ] if tf.nest.is_nested( states ) else z_t
        
        self.step.assign( tf.math.floormod( self.step + 1 ,  self.state_size ) )
...
@tf.keras.utils.register_keras_serializable( 'hopf_layer' )
class HopfRNNLayer( tf.keras.layers.Layer ):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super( HopfRNNLayer , self ).__init__( **kwargs )
    ...
    def build( self , input_shape ):
        ...
        super( HopfRNNLayer , self ).build( input_shape )
...
#        _print( 'outputs' , outputs )
    ...
    def get_config(self):
        config = super( HopfRNNLayer , self ).get_config()
