...
#        zz = tf.math.multiply( z , tf.math.conj( z ) )
        z2 = tf.math.pow( tf.math.abs( z ) , 2 )
        zz = tf.complex( z2 , tf.zeros_like( tf.math.imag( z2 ) ) )
#        _print( 'zz' , zz )
...
#        dr = ( a - r*r ) * r
    ...
    @tf.function
    def mke_unt( self , c ):
        return c / tf.math.abs( c )
    
    @tf.function
    def cpx_hopf_ODE( self , z , a , b , stp , unts ):
        
#        uz , ua = self.mke_unt( z ) , self.mke_unt( a )
#        ma = tf.math.reduce_min( tf.math.angle( uz * tf.math.conj( ua ) ) )
#        ma = tf.cast( tf.math.abs( ma ) , dtype = tf.float32 )
...
#        '''
        ...
        tst = tf.math.reduce_mean( ode.states , 0 )
        re_zt , im_zt = tf.unstack( tst )
#        _print( 'ret' , ret )
#        _print( 'imt' , imt )
...
#        z_0 , a_0 , b_0 = z , a , b
        z_t = self.cpx_hopf_ODE( z_0 , a_0 , b_0 , stp , unts )
#        return z_t
