...
#        ''' Creates a unitary weight matrix from
    ...
    #        print('self.A',self.A)
            ...
            self.B = self.add_weight(
                ...
                shape=( self.unit_size, self.unit_size ),
                dtype=self.dtype,
...
#        if isinstance(states , (tuple , list)): tf.print('states',type(states),' len:',len(states))
#        if isinstance(inputs , (tuple , list)): tf.print('inputs',type(inputs),' len:',len(inputs))
...
#        _print('C new_inputs',new_inputs)
        
#        with tf.name_scope(self.name):
        input_map = self.map_hermitian_kernel( new_inputs , self.A)
#        _print('input_map',input_map)
        
        state_map = self.map_hermitian_kernel( prev_state , self.B )
#        _print('state_map',state_map)
        
        new_state = state_map
#        _print('new_state',new_state)
        
        output = self.activation( input_map )
...
#        for cell in self.cells: print(cell.name,':\n',cell.A)
#        exit(0)
        
        self.global_state = tf.Variable(
            initial_value=tf.zeros( shape=( self.unit_count , 1 , self.unit_size ) , dtype=self.dtype ),
            shape=( self.unit_count , 1 , self.unit_size ),
            dtype=self.dtype,
            trainable=False,
            name='global_state'
        )
    ...
    # Change in global state from one timestep to another.
...
#        _print('inputs',inputs)
        ...
        _inputs = tf.cast( inputs, dtype=self.dtype )
...
#        tf.print('cell_outs:',cell_outs.shape)
#        tf.print('cell_states:',cell_states.shape)
#        tf.print('global_state:',global_state.shape)
...
#        _print( 'global_state' , self.global_state )
        ...
        outputs = tf.math.reduce_mean( cell_outs , axis=[1])
    ...
    def local_rnn_call( self , cells , inputs , state , training=False , agg_func=tf.math.reduce_mean ):
        ...
        # Make into inputs list
        if not tf.nest.is_nested(state): nest_state = tf.unstack(state)
        elif len(state) == 1: nest_state = tf.unstack(state[0])
        assert len(nest_state) > 0, 'Inputs given to local_rnn_call was length 0.'
        ...
        assert len(cell_call_fns)==len(nest_state), 'Local_rnn_call can only work if the state\'s row-dimesnion is the same as the cell units.'
...
#        _tmp_out , _tmp_state = cell_call_fn( inputs[0], state , training=False )
        _tmp_out , _tmp_state = cell_call_fns[0]( inputs[0], [nest_state[0], state] , training=False )
        ...
        _tmp_state = _tmp_state[0] if tf.nest.is_nested(_tmp_state) else _tmp_state
        ...
        def _step( time , output_ta_t, state_ta_t , *state ):
            ...
            # Call Cell
            called_cells = tf.nest.map_structure(
                lambda call_fn , curr_s : call_fn( curr_input , curr_s , training=training ),
                cell_call_fns , tf.unstack(state[0])
            )
