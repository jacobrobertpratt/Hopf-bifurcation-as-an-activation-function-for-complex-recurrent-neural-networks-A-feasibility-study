...
#        assert self.unit_size == self.cells.unit_size, 'Cell unit size and layer unit size are miss-matched. Check code.'
        ...
        self.global_state = tf.Variable(
            initial_value=tf.zeros( shape=( self.unit_count , self.unit_size ) , dtype=self.dtype ),
            shape=( self.unit_count , self.unit_size ),
            dtype=self.dtype,
            trainable=False,
    ...
    def set_global_state( self , curr_state , all_states):
        ...
        new_states = curr_state + tf.math.divide_no_nan(
                                        tf.math.reduce_mean( all_states , axis=0 ),
                                        tf.math.reduce_mean( all_states )
                                    )
