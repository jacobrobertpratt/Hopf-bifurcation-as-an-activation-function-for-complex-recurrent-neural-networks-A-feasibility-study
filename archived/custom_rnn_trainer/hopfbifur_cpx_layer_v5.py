...
class HopfBifurCpxRNNLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        lastout, outputs, states = self.backend_rnn_call(
            self.cell,
            inputs,
            self.states,
            training=training,
...
#        '''
        ...
        rnn_outputs = self.local_rnn_call( inputs , self.states)
        
        # TODO: Apply aggregation stuff and put back into states.
        
#        self.states.assign_add( tf.reshape( states , self.states.shape ) )
...
#        _print( 'outputs' , outputs )
    ...
    def local_rnn_call( self , cells , inputs , states ):
        
        _print( 'inputs' , inputs )
        
        time_steps = inputs.shape[0]
        time_step_t = inputs.shape[1]
        
        tf.print( 'time_steps' , time_steps )
        tf.print( 'a_timesteps' , a_timesteps )
        
        
        flat_inputs = tf.unstack(inputs)
        
        assert len(flat_inputs) == len(flat_states), 'Inputs and states must have the same batch dimension.'
        
        input_tensarr = []
        for i, inp in enumerate(flat_inputs):
            input_tensarr.append(
                tf.TensorArray(
                    dtype=inp.dtype,
                    size=a_timesteps,
                    tensor_array_name='input_tensarr_%s' % i
                )
            )
        input_tensarr = tuple(input_tensarr)
        tf.print('input_tensarr',input_tensarr)
        
        states_tensarr = []
        for i, inp in enumerate(flat_states):
            flat_states.append(
                tf.TensorArray(
                    dtype=inp.dtype,
                    size=a_timesteps,
                    tensor_array_name='state_tensarr_%s' % i
                )
            )
        states_tensarr = tuple(states_tensarr)
        tf.print('states_tensarr',states_tensarr)
