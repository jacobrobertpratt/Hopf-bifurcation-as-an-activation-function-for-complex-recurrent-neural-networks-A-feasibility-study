...
#        '''
        ...
        U = Unitary( name = 'U' , save = do_save , conjsym = True )
        U = U( ( self.state_size , self.state_size ) , self.dtype )
        adjU = tf.linalg.adjoint( U )
        
        def unt_init( shape , dtype ):
            return tf.linalg.logm( U )
        self.U = self.add_weight(
            name = 'U_skew_' + self.name,
            shape = ( self.state_size , self.state_size ),
            dtype = self.dtype,
            initializer = unt_init,
            trainable = do_train
        )
#        TU = tf.linalg.logm( self.U )
#        evls = tf.linalg.eigvals( self.U )
#        _print( 'U' , self.U )
#        _print( 'U evls' , evls )
#        _print( 'trc U' , tf.linalg.trace( self.U ) )
#        _print( 'det U' , tf.linalg.det( self.U ) )
#        exit(0)
        
        def cos_init( shape , dtype ):
            return ( U + adjU ) / 2.
        self.C = self.add_weight(
            name = 'C_herm_' + self.name,
            shape = ( self.state_size , self.state_size ),
            dtype = self.dtype,
            initializer = cos_init,
            trainable = do_train
        )
...
#        _print( 'C evls' , evls )
        ...
        def sin_init( shape , dtype ):
            return ( U - adjU ) / 2.j
        self.S = self.add_weight(
            name = 'S_skew_' + self.name,
            shape = ( self.state_size , self.state_size ),
            dtype = self.dtype,
            initializer = sin_init,
            trainable = do_train
        )
...
#        U = U( ( self.state_size , self.state_size ) , self.dtype )
        ...
        def unt_init( shape , dtype ):
            U = Unitary( name = 'U' , save = do_save , conjsym = True )
            return U( ( self.state_size , self.state_size ) , self.dtype )
...
#        S = self.S
        H = self.H
        G = self.G
#        nones = self.ones * (-1.+0.j)
        
#        nz , _ = tf.linalg.normalize( z , ord = 2 )
#        nx , _ = tf.linalg.normalize( x , ord = 2 )
        
        z_i = self.hermitian_map( G , z )
        x_i = self.hermitian_map( H , x )
#        _print( 'z_i' , z_i )
#        _print( 'x_i' , x_i )
        
#        uz = z / ( tf.math.conj( z ) * z )
        z_t = z
        x_t = z_i + x_i
#        _print( 'z_t' , z_t )
#        _print( 'x_t' , x_t )
...
#        tf.debugging.check_numerics( tf.math.real( x_t ) , 'Cell -> x_t element is nan' )
        ...
        x_t = tf.reshape( x_t , x_in.shape )
        z_t = tf.reshape( z_t , z_in.shape )
...
#        '''
        if do_save:
            ...
            if not os.path.exists('state_layer.npy'):
                state_init = np.random.rand( self.state_size )
                state_init = np.arange( self.state_size )*2 / self.state_size
...
#        exit(0)
    ...
    @tf.function
    def make_base( self , z , size ):
        ang = tf.math.angle( z ) 
        stp = tf.math.reduce_min( tf.math.abs( ang ) )
        rng = tf.cast( tf.range( size ) * stp , dtype = z.dtype )
        rng = tf.expand_dims( rng , 0 )
        base = tf.math.pow( tf.math.exp( 1.j * tf.math.angle( z ) ) , rng )
        base , _ = tf.linalg.normalize( base , ord = 2 , axis = [ -2 , -1 ] )
        return base
...
#        tf.print( '\n'*8+'=='*25+' Layer '+'=='*25+'\nCount:',self.cnt[0],'\n' )
        ...
        _ibase = self.make_base( _state , self.input_size )
...
#        '''
        ...
        clastout , coutput , cstate = rnn_return
        '''
        clastout = tf.unstack( _input )[-1]
        coutput = _input
        cstate = _state
#        '''
        
#        tf.print('\n'+' *'*15+' RNN Returned '+'* '*15+'\n')
#        _print( 'clastout' , clastout )
#        _print( 'coutput' , coutput )
#        _print( 'cstate' , cstate )
#        _print( 'L self.state' , self.states )
#        _print( 'L self.manif' , self.manif )
        
#        tf.debugging.check_numerics( tf.math.real( clastout ) , 'a clastout element is nan' )
#        tf.debugging.check_numerics( tf.math.real( coutput ) , 'a coutput element is nan' )
#        tf.debugging.check_numerics( tf.math.real( cstate ) , 'a cstate element is nan' )
        
#        cstate = tf.reshape( cstate , _state.shape )
#        coutput = tf.reshape( coutput , _input.shape )
#        _print( 'coutput' , coutput )
#        _print( 'cstate' , cstate )
        
        self.state.assign( cstate )
        
        _obase = self.make_base( cstate , self.output_size )
        _output = tf.linalg.matmul( coutput , _obase , adjoint_a = True )
