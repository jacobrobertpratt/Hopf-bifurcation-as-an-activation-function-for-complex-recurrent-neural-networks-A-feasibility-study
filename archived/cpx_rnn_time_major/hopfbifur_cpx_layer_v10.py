...
class HopfBifurCpxRNNCell(tf.keras.layers.Layer):
    ...
    def build( self, input_shape ):
        ...
        self.A = self.add_weight(
            ...
            shape = ( sz , sz ),
            dtype = self.dtype,
            initializer = Hermitian( name = 'A' , save = do_save , conjsym = do_conjsym ),
            trainable = do_train
        )
        
        self.ones = tf.constant( tf.ones( shape = ( sz , 1 ), dtype = self.dtype ), dtype = self.dtype )
        
        rnginit = tf.range( self.input_size )
        rnginit = tf.cast( tf.reshape( [ rnginit ] , [ 1 , sz ] ) , dtype = self.dtype )
...
#        _print( 'x_in' , x_in )
        ...
        base = tf.math.pow( z / tf.math.abs( z ) , self.inrng )
        tstx = tf.linalg.matmul( base , x )
        innr = tf.linalg.matmul( tstx , z , adjoint_a = True )
        _print( 'innr' , innr )
#        _print( 'x' , x )
#        _print( 'z' , z )
        
#        A = self.A
#        Az = self.hermitian_map( A , z )
#        Ax = self.hermitian_map( A , x )
...
#            _print( 'A) J' , J )
            ...
            nrmJ = tf.math.sqrt( tf.linalg.trace( tf.linalg.matmul( dU , dU , adjoint_a = True ) ) ) 
#            _print( 'nrmJ' , nrmJ )
            
            J = tf.math.divide_no_nan( J , nrmJ )
#            _print( 'A) J' , J )
...
#       '''
        ...
        rnd = (rnd + rnd[::-1])/2.
        state_init = tf.expand_dims( tf.math.exp( 1.j*rnd ) , 0 )
