...
#        tf.print('\n'+'- '*20+' Cell '+'- '*20+'\nCount:',self.cnt[0],'\n')
        ...
        z_T = tf.linalg.matrix_transpose( z )
        h_T = tf.linalg.matrix_transpose( h )
...
#            V = ( tf.linalg.matmul( x2 , gL2 , adjoint_b=True ) + tf.linalg.matmul( gL2 , x2 , adjoint_b=True ) ) / 2. 
            ...
            V = tf.math.divide_no_nan(
                V, tf.math.sqrt( tf.linalg.trace( tf.linalg.matmul( V , V , adjoint_a = True ) ) )
            )
...
#            _print( 'GV' , GV )
            ...
            gx2 = tf.linalg.matmul( Dv_H2 , gL2 , adjoint_a=True )
...
#        _print( 'L B) out_map' , out_map )
        ...
        tf.debugging.assert_less( max_img_val , 1.e-6, 'imaginary values went above 1.e-5 issue with recurrance.' )
