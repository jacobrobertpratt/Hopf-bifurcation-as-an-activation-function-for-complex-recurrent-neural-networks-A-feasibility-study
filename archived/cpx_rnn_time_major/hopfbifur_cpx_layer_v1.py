...
class HopfBifurCpxRNNCell(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        def get_T_init(shape,dtype):
            btsz = shape[0]
            insz = shape[-1]
            T_init = np.asarray( [ np.exp(-2.j*np.pi*np.arange(insz+1)/(insz+1))[1::] ] * btsz )
            T_init = tf.reshape( tf.cast( T_init.copy() , dtype=dtype ) , [ btsz , 1 ,insz ] )
            return T_init
        
        self.T = self.add_weight(
            name='T_wgt'+self.name,
            shape=( batch_size , 1 , input_size ),
            dtype=self.dtype,
            initializer=get_T_init,
            trainable=False
        )
    ...
    def proj_inputs_to_state( self , t , z , x ):
        z_cT = tf.transpose( z , conjugate=True )
        x_T = tf.transpose( x )
        zt = tf.linalg.matmul( z_cT , t ) / self.sqrt_insize
        prj_ztx = tf.linalg.matmul( zt , x_T )
        return tf.transpose( prj_ztx )
...
#        tf.print('\n'+'- '*20+' Cell '+'- '*20+'\nCount:',self.cnt[0],'\n')
        ...
        flat_T = tf.unstack( self.T )
        
        proj_input = tf.nest.map_structure(
            self.proj_inputs_to_state ,
            flat_T,
            flat_states,
            flat_inputs
        )
        ...
        flat_inputs_map = tf.nest.map_structure( self.map_hermitian_kernel, proj_input , flat_A)
        flat_states_map = tf.nest.map_structure( self.map_hermitian_kernel, flat_states , flat_B)
        
        mapped_inputs = tf.reshape( tf.stack( flat_inputs_map ) , prev_states.shape )
        mapped_states = tf.reshape( tf.stack( flat_states_map ) , prev_states.shape )
        
        new_hidden = mapped_inputs + mapped_states
        _print( 'new_hidden' , new_hidden )
        
        new_output = self.activation( new_hidden )
        _print( 'new_output' , new_output )
        
        # check for shape compatibility & re-nest if the previous one was nested.
        output = tf.reshape( new_output , prev_states.shape )
        output = [ output ] if tf.nest.is_nested( inputs ) else output
        
        new_state = tf.reshape( new_hidden , prev_states.shape )
...
#        self.time_major = True if time_size > 1 else False
        ...
        # Global State
        root = np.exp(-2.j*np.pi*np.arange( self.state_size + 1) / (self.state_size + 1) )
        root = root[1::].reshape(-1,1) #/ np.sqrt( self.state_size )
        
        global_state_init = tf.cast( ( root ** np.arange(1,batch_size+1) ).copy() , dtype=self.dtype )
        global_state_init = tf.reshape( global_state_init , [ batch_size , self.state_size ] )
        
        self.global_state = tf.Variable(
            initial_value=global_state_init,
            shape=( batch_size , self.state_size ),
            dtype=self.dtype,
            trainable=False,
            name='global_state'
        )
#        _print('global_state', self.global_state )
...
#        _print('inputs',inputs)
        ...
        cell_outs , _ , cell_states = rnn_return
        
        tf.ensure_shape( cell_states , self.global_state.shape )
        tf.ensure_shape( cell_outs , self.global_state.shape )
        
#        _print( 'rnn: cell_outs' , cell_outs )
#        _print( 'rnn: cell_states' , cell_states )
        
        tf.debugging.check_numerics( tf.math.real(cell_outs) , 'a cell_outs element is nan')
        tf.debugging.check_numerics( tf.math.real(cell_states) , 'a cell_states element is nan')
        ...
        outputs = tf.math.reduce_mean( tf.math.abs( cell_outs ) )
