...
class HopfBifurCpxRNNCell(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.A = self.add_weight(
            ...
            dtype=self.dtype,
            initializer=RandomUnitComplex( input_size , self.state_size ),
            trainable=True
        )
        ...
        self.B = self.add_weight(
            ...
            dtype=self.dtype,
            initializer=RandomUnitComplex( self.state_size , self.state_size ),
            trainable=True
        )
        '''
...
#        input_map = tf.linalg.matmul( new_inputs , self.A )
#        state_map = tf.linalg.matmul( prev_states , self.B )
        
#        new_state = input_map + state_map
#        new_output = self.activation( new_state )
#        _print( 'input_map' , input_map )
#        _print( 'state_map' , state_map )
#        _print( 'new_state' , new_state )
#        _print( 'new_output' , new_output )
        
        output_map = new_input
        state_map = prev_state
        
        # check for shape compatibility & re-nest if the previous one was nested.
        ret_value = [ output_map ] if tf.nest.is_nested( inputs ) else output_map
        ret_state = [ state_map ] if tf.nest.is_nested( states ) else state_map
...
class HopfBifurCpxRNNLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        state_init = np.exp(-2.j*np.pi*state_init)*state_init
        ...
        rngshp = self.output_size
        rnginit = [np.arange(1,rngshp+1)]
        rnginit = tf.reshape( rnginit , [ 1 , 1 , rngshp ])
    ...
    @tf.function
    def project_state_to_state( self , state1 , state2 ):
        state1_T = tf.linalg.matrix_transpose( state1 , conjugate=True )
        state2_T = tf.linalg.matrix_transpose( state2 , conjugate=True )
        state1_norm = tf.linalg.matmul( state1 , state1_T )
        proj = tf.math.divide_no_nan( tf.linalg.matmul( state1_T , state2 ), state1_norm )
        proj_state = tf.linalg.matmul( proj, state2_T )
        return tf.reshape( proj_state , state1.shape )
        
        
    @tf.function
    def generate_bases( self , state , range , do_conj=False ):
        outroot = tf.math.exp( -2.j*np.pi*tf.math.angle( state ))
        outroot = tf.linalg.matrix_transpose( outroot , conjugate=do_conj )
        bases = tf.math.pow( outroot , range )
        bases = tf.math.divide_no_nan( bases, tf.math.sqrt( range[-1] ) )
        return bases
        
        
    @tf.function
    def project_inputs( self , state , input , range ):
        state_bases = self.generate_bases( state , range , do_conj=False )
        proj_inpt = tf.linalg.matmul( state_bases , tf.linalg.matrix_transpose( input , conjugate=True ) )
        proj_inpt = tf.reshape( proj_inpt , [input.shape[0] , 1 , state.shape[-1]] )
        return proj_inpt
...
#        _print('state',self.state)
        ...
        proj_inputs = self.project_inputs( self.state , cast_inputs , self.inrng )
...
#        _print( 'self.state' , self.state )
        ...
        new_state = self.project_state_to_state( self.state , cell_state )
        new_state = tf.reshape( new_state , self.state.shape )
        self.state.assign( new_state )
#        _print( 'new_state' , new_state )
        
        state_bases = self.generate_bases( new_state , self.outrng , do_conj=True )
        
        out_map = tf.linalg.matmul( seq_out , state_bases )
        _print( 'out_map[0]' , out_map[0] )
