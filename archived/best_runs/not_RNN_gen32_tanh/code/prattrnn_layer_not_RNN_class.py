...
#        self.activation = HopfBifur()
        ...
        # Input Kernel #
        kern_wgt_ini = gen_complex_random_weight_matrix(input_shape[-1],self.size)
        ...
        # Recurrent Kernel #
        rec_kern_wgt_ini = gen_unitary_weight_matrix(self.size)
        ...
        # Hidden State # 
        hid_state_ini = tf.zeros(shape=input_shape, dtype=self.dtype)
        self.hidden_state = tf.Variable(
            initial_value=hid_state_ini,
            trainable=True,
            name='hidden_state'
        )
...
#        _print('prev_state',prev_state)
        ...
        kern_map = tf.linalg.matmul(input, self.kernel)     # (1 x n)
...
#        tf.ensure_shape(new_state, prev_state.shape)
...
class PrattRNNLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        if self.cells is None:
            self.cells = PrattRNNCell(dtype=self.dtype)
            self.cells.build(input_shape[1::])
        
#        state_shape = [input_shape[1],input_shape[-1]]
        ini_zeros = tf.zeros(shape=input_shape, dtype=self.dtype)
