...
#                wgt = (wgt + tf.math.conj(wgt)[::-1])/2.
#                wgt = wgt / tf.math.pow(tf.linalg.det(wgt),(1/in_sz))
#                wgt = wgt / np.sqrt(t_sz)
#                wgt = tf.linalg.expm(wgt)
#                _print('wgt',wgt)
#                _print('wgt Ichk',tf.math.conj(wgt).T @ wgt)
#                _cpx_pdet('wgt',wgt)
#                _cpx_ptrace('wgt',wgt)
#                exit(0)
                return wgt
...
#        z = make_range_vec(in_sz,'z',do_save=_save)
#        z = tf.cast(z,dtype=tf.float64)
#        z = z / (tf.math.reduce_max(z) + 1.)
#        z = (z + z[::-1])/2.
        z = self.A @ z
        self.z = tf.Variable(
                              initial_value=z,
...
#    @tf.function
    ...
    def unitary_map(self,M,v):
        ...
        def grad(gL):
            ...
            gM = 0.1*(tf.math.conj(M).T @ ((J - M @ tf.math.conj(J).T @ M)/2.))
            gv = tf.zeros_like(v)
            return gM, gv
        return Mv, grad
    ...
    # C = (U + tf.math.conj(U).T)/2.    
    #   &    S = (U - tf.math.conj(U).T)/2.j
