...
from my_layer_single_A import MyRNN, MyLoss, MyModel
...
#            tf.print('count:',self.count)
            ...
            if ('U' in variable.name) or ('W' in variable.name):
                # Used when the 'variable' is a unitary matrix
                variable.assign(variable @ tf.linalg.expm(-lr*gradient))
            else:
                # When the variable is hermitian matrix
                dV = tf.linalg.expm(1.j*((1.j*tf.linalg.logm(variable)) + 0.001*gradient))
                variable.assign(dV)
...
#            print(ini_U,'\nshape:',ini_U.shape,'  dtype:',ini_U.dtype,'\n')
...
#            print(ini_W,'\nshape:',ini_W.shape,'  dtype:',ini_W.dtype,'\n')
...
#            print('Ichk [W]:\n',np.round(Ichk_W,6),'\n')
        ...
        if 'U' in fin_wgt_dict:
            ...
            print(fin_U,'\nshape:',fin_U.shape,'  dtype:',fin_U.dtype,'\n')
...
#            print('Ichk [U]:\n',np.round(Ichk_U,6),'\n')
        if 'W' in fin_wgt_dict:
            ...
            print(fin_W,'\nshape:',fin_W.shape,'  dtype:',fin_W.dtype,'\n')
