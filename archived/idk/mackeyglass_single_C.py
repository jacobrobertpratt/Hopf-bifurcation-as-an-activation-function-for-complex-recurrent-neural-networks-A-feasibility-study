...
from my_layer_single import MyRNN, MyLoss, MyModel
...
class MyOptimizer(tf.keras.optimizers.Optimizer):
    ...
    def update_step(self, gradient, variable):
        ...
        lr = tf.cast(self.learning_rate[0],dtype=self.dtype)
 
        if ('U' in variable.name) or ('V' in variable.name):
...
#    myrms = tf.keras.optimizers.RMSprop(learning_rate=0.001) # ,momentum=0.001,rho=0.9)
#    myrms = tf.keras.optimizers.Adagrad(learning_rate=0.001)
    myrms = MyOptimizer(learning_rate=0.001)
...
#   tf.keras.losses.CosineSimilarity(axis=-1,reduction=losses_utils.ReductionV2.AUTO,name='cosine_similarity')
    ...
    myaccu = tf.keras.metrics.MeanSquaredLogarithmicError()
    
    model.compile(loss=myloss,optimizer=myrms,metrics=[myloss,myaccu]) #'loss']) #,'accuracy'])
    print("model.summary:\n",model.summary(),'\n')
    ...
    erlstp_chkpt = tf.keras.callbacks.EarlyStopping(
                                                     monitor='loss',
                                                     min_delta=1.e-4,
                                                     patience=10,
                                                     verbose=0,
    ...
    mod_fit = model.fit(
                         ...
                         callbacks=[loss_chkpnt] # ,erlstp_chkpt]
                        )
