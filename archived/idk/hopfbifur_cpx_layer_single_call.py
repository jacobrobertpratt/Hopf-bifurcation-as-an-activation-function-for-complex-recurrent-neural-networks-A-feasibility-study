...
#        '''
    ...
    def map_hermitian_kernel(self, input, kernel):
        ...
        output = tf.einsum('bij,bjk->bik', input, kernel)
    ...
    def map_unitary_kernel(self, input, kernel):
        ...
        def grad(gL):
            ...
            J = tf.einsum('bij,bjk->bik', gL_cT, input)
            J_cT = tf.math.conj(tf.transpose(J,[0,2,1]))
            lMJ = tf.einsum('bij,bjk->bik', kernel, J_cT)
            rMJ = tf.einsum('bij,bjk->bik', lMJ, kernel)
            J_sub_rMJ = tf.math.divide( tf.math.subtract(J, rMJ), 2.)
            lMJcT = tf.math.conj(tf.transpose(kernel,[0,2,1]))
            gk = tf.einsum('bij,bjk->bik', lMJcT, J_sub_rMJ)
            gi = tf.einsum('bij,bjk->bik', gL, lMJcT)
            return gi, gk
        return output, grad
...
#        mapped_inputs = self.map_unitary_kernel( inputs, self.U)
        
        mapped_states = self.map_hermitian_kernel( states, self.H)
#        mapped_states = self.map_unitary_kernel( states, self.W)
        
        new_states = mapped_inputs + mapped_states
        
        act_out = new_states # self.activation(new_states)
#        tf.print('C out:',tf.math.real(act_out),'\n',tf.math.imag(act_out),'\n')
        
        return act_out, new_states
...
class HopfBifurCpxRNNLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.state = tf.Variable(
            initial_value=UnitaryState(state_dim)([batch_dim], dtype=self.dtype),
            trainable=False,
            name='state',
            dtype=self.dtype
        )
...
#        exit(0)
    ...
    def call(self, inputs, training=None):
        ...
        input_map = tf.linalg.matmul( input , self.B )
        
        '''
        input_map = tf.linalg.matmul( input , tf.transpose( self.B , [0,2,1] ) )
        _print( 'B) input_map' , input_map )
        '''
        
#        new_output, new_state = self.cell.__call__( fft_in, self.states, training=training)
        
        outputs = tf.keras.activations.tanh( input_map )
