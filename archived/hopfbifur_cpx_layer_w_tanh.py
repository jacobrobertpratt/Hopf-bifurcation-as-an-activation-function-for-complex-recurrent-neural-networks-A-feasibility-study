...
class HopfBifurCpxRNNCell(tf.keras.layers.Layer):
    ...
    def build( self, input_shape ):
        ...
        ssz = self.state_size // 2 + self.state_size % 2
        ...
        self.A = self.add_weight(
            ...
            shape = ( ssz , ssz ),
            dtype = self.dtype,
            initializer = Hermitian( name = 'A' , save = self.do_save ), # , conjsym = self.do_conjsym ),
            trainable = do_train
        )
#        _print( 'A' , self.A )
        
        def gen_rnd_cpx( shape , dtype ):
            shp = list( shape )
            W = np.random.rand( shp[0] , shp[1] ) + 1.j*np.random.rand( shp[0] , shp[1] )
            return tf.reshape( tf.cast( W , dtype = dtype ) , shape )
            
        self.Q = self.add_weight(
            name = 'Q' + self.name,
            shape = ( ssz , ssz ),
            dtype = self.dtype,
            initializer = gen_rnd_cpx,
            trainable = do_train
        )
...
#        C , S = tf.unstack( self.U )
#        _print( 'C' , C )
#        _print( 'S' , S )
#        exit(0)
        
        self.onessz = tf.constant( tf.ones( shape = ( ssz , 1 ), dtype = self.dtype ), dtype = self.dtype )
...
#        self.test_herm_mat( C )
#        self.test_herm_mat( S )
#        self.test_herm_mat( A )
        
#        U = C + 1.j*S
#        U = self.test_unit_mat( C + 1.j*S )
...
#        '''
        zn_j = self.hermitian_map( C , nz ) + 1.j*self.hermitian_map( S , nz )
        xn_j = self.hermitian_map( C , nx ) + 1.j*self.hermitian_map( S , nx )
        '''
        zn_j = tf.linalg.matmul( Q , nz )
        xn_j = tf.linalg.matmul( R , nx )
#       '''
        
        '''
        zn_j = self.hermitian_map( C , nz ) + self.skewherm_map( 1.j*S , nz )
        xn_j = self.hermitian_map( C , nx ) + self.skewherm_map( 1.j*S , nx )
#       '''
#        _print( 'zn_j' , zn_j )
#        _print( 'xn_j' , xn_j )
        
        '''
        zn_j = ( self.hermitian_map( H , nz ) + self.hermitian_map( tf.linalg.matrix_transpose( H ) , nz ) ) / 2.
        xn_j = ( self.hermitian_map( G , nx ) + self.hermitian_map( tf.linalg.matrix_transpose( G ) , nx ) ) / 2.
        
#        trc_z = 2*tf.math.real( zn_j ) + 0.j
#        det_z = tf.math.real( zn_j * tf.math.conj( zn_j ) ) + 0.j
#        _print( 'trc_z' , trc_z , summarize = -1 )
#        _print( 'det_z' , det_z , summarize = -1 )
        
        izn_j = 1.j*tf.math.imag( zn_j )
        alpha = ( ( 2*tf.math.real( zn_j ) )**2 ) + izn_j
        beta = (-4.0 * tf.math.real( zn_j*tf.math.conj( zn_j ) ) ) - izn_j
        _print( 'alpha' , alpha , summarize = -1 )
        _print( 'beta' , beta , summarize = -1 )
        
        zz = tf.math.real( tf.math.conj( zn_j ) * zn_j ) + 0.j
        _print( 'zz' , zz )
        
        dz = ( alpha + beta * zz ) * zn_j
        _print( 'dz' , dz )
        
        xn_j = self.activation( xn_j , alpha , beta )
#        _print( 'xn_j' , xn_j , summarize = -1 )
        '''
        
#        inrN = tf.linalg.matmul( nz , nx , adjoint_a = True )
#        _print( 'inrN' , inrN )
        
        '''
        Cz = self.hermitian_map( C , nz )
        Cx = self.hermitian_map( C , nx )
        _print( 'Cz' , Cz , summarize = -1 )
#        _print( 'Cx' , Cx , summarize = -1 )
        
        Sz = self.skewherm_map( 1.j*S , nz )
        Sx = self.skewherm_map( 1.j*S , nx )
        _print( 'Sz' , Sz , summarize = -1 )
#        _print( 'Sx' , Sx , summarize = -1 )
        
        Uz = Cz + Sz
        _print( 'Uz' , Uz )
        _print( 'U @ z' , tf.linalg.matmul( U , nz ) )
        
        Ux = Cx + Sx
#        self.test_unit_mat( Ux )
#        _print( 'Uz' , Uz )
#        _print( 'Ux' , Ux , summarize = -1 )
        
#        det_UzUx = Uz * tf.math.conj( Uz )
#        _print( 'det_UzUx' , det_UzUx )
        
#        inv_z1 = tf.math.conj( Uz ) / det_UzUx
#        _print( 'inv_z1' , inv_z1 )
        
#        inrU = tf.linalg.matmul( Uz , Ux , adjoint_a = True )
#        _print( 'inrU' , inrU )
        '''
...
#        _print( 'ang( x )' , tf.math.angle( x ) )
        
        # state_size , sqrssz , sqrisz
        z_k = tf.concat( [ z0_j , zn_j ] , 0 ) / self.sqrssz
        x_k = tf.concat( [ x0_j , xn_j ] , 0 ) / self.sqrssz
#        _print( 'z_k' , z_k )
#        _print( 'x_k' , x_k )
#        _print( 'z_k' , z_k , summarize = -1 )
#        _print( 'x_k' , x_k , summarize = -1 )
#        _print( 'ang( z_k )' , tf.math.angle( z_k ) )
#        _print( 'ang( x_k )' , tf.math.angle( x_k ) )
        
#        z_t = z + z_k
#        x_t = x + x_k * tf.keras.activations.tanh( z_t )
        z_t = z_k + x_k
        x_t = tf.keras.activations.tanh( z_t )
#        _print( 'z_t' , z_t )
        _print( 'x_t' , x_t )
#        _print( 'ang( z_t )' , tf.math.angle( z_t ) )
#        _print( 'ang( x_t )' , tf.math.angle( x_t ) )
        
#        gtrchk = tf.math.reduce_min( tf.math.real( tst1 ) )
#        tf.debugging.assert_greater_equal( gtrchk , 0. , 'RnnCell - > tst1 less than zero.' )
#        _print( 'gtrchk' , gtrchk )
        
        '''
        matvec = self.mke_vec_to_mat( z_t )
        
        z1 , m1 = z_t[1] , matvec[1]
        
        trc = tf.linalg.trace( m1 )
        det = tf.linalg.det( m1 )
        _print( 'm1' , m1 )
        _print( 'trc' , trc )
        _print( 'det' , det )
        
        chk_m1 = ( trc**2 ) - (4*det )
        _print( 'chk_m1' , chk_m1 )
        
        inr_m1 = tf.linalg.matmul( m1 , m1 , adjoint_a = True )
        _print( 'inr_m1' , inr_m1 )
        
        inr_z1 = z1*tf.math.conj(z1)    # Equal to det( m1 )
        _print( 'inr_z1' , inr_z1 )
        
        trc_z1 = tf.math.real( z1 )*2   # Equal to trace( m1 )
        _print( 'trc_z1' , trc_z1 )
        
        chk_z1 = ( (2*tf.math.real(z1) )**2 ) - ( 4*z1*tf.math.conj(z1) )
        _print( 'chk_z1' , chk_z1 )
        '''
...
class HopfBifurCpxRNNLayer(tf.keras.layers.Layer):
    ...
    def __init__( self , state_size = None , output_size = None , activation = None , **kwargs ):
    ...
    def build( self , input_shape ):
        ...
        if self.state_size is None: self.state_size = self.input_size
        if self.output_size is None: self.output_size = self.state_size
        ...
        state_init = tf.signal.rfft( state_init ).reshape( -1 , 1 ) / self.sqrssz
...
#        _print( 'state' , self.state , summarize = -1 )
#        exit(0)
        
        self.osz = self.output_size // 2 + 1
        self.ssz = self.state_size // 2 + 1
        
        szdf = self.osz - self.state.shape[0]
        szdf = tf.reshape(
            tf.cast( [[0]*szdf]*bsz , dtype = self.dtype ),
            [ self.batch_size , 1 , szdf ]
        )
        self.padding = tf.constant( szdf , self.dtype )
#        _print( 'self.padding' , self.padding )
#        exit(0)
...
#        _print( 'input' , input )
        ...
        _input = tf.signal.rfft( input ) / self.sqrisz
        _input = tf.linalg.matrix_transpose( _input[::,::,0:self.ssz] )
...
#        '''
        ...
        _cout = tf.linalg.matrix_transpose( _cout )
        _cout = tf.concat( [ _cout[::,::,0:self.osz] , self.padding ] , -1 )
...
#        tf.debugging.check_numerics( tf.math.real( _coutput ) , 'a _coutput element is nan' )
        ...
        _output = tf.signal.irfft( _cout ) * self.sqrisz
