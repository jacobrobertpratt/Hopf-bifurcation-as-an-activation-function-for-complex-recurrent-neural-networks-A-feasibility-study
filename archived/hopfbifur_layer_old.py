...
@tf.keras.utils.register_keras_serializable('hopfbifur_layer')
class HopfBifurRNNCell(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        super( HopfBifurRNNCell , self ).__init__( **kwargs )
...
#        ''' Creates a unitary weight matrix from
        ...
        super(HopfBifurRNNCell, self).build(input_shape)
    ...
    def call(self, input_tuple, training=False):
        ...
        output = self.activation( new_state , tf.math.pow( new_state , 2 ) , tf.zeros_like( new_state ) )
    ...
    def get_config(self):
        base_config = super(HopfBifurRNNCell, self).get_config()
        config = {
                    "units": self.units,
...
@tf.keras.utils.register_keras_serializable('hopfbifur_layer')
class HopfBifurRNNLayer(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        super(HopfBifurRNNLayer, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        if self.cells is None:
            self.cells = HopfBifurRNNCell(size=self.size, dtype=self.dtype)
        ...
        super( HopfBifurRNNLayer , self ).build( input_shape )
    ...
    def get_config(self):
        base_config = super(HopfBifurRNNLayer, self).get_config()
